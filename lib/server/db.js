...
/** Create tables if missing (safe to call on every request; runs once per cold start). */
...
export async function saveUserBundle(sql, userId, patch) {
  ...
  // Pass JS values directly — Neon serializes JSONB; JSON.stringify would double-encode.
  await sql`
